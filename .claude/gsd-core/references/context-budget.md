...
The keys live in `.claude/settings.json` (project) or `/Users/wilsonsmacmini/Documents/Code/finally/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/wilsonsmacmini/Documents/Code/finally/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
